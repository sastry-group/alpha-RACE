...
parser.add_argument('--exp_name', default='fig5', type=str, help='Name of the experiment')
...
class CarNode(Node):
    def __init__(self):
        ...
        self.curr_sf1_opp = 0.4
        self.curr_sf2_opp = 0.1
        self.blocking_factor_opp = 0.
    ...
    def timer_callback(self):
        ...
        if self.i > EP_LEN :
            ...
            print("Progresses:", len(self.dataset),s,s_opp,s_opp-s)
            self.diffs.append(s_opp-s)
            self.last_i = -1
            self.last_i_opp = -1
            self.last_i_opp1 = -1
            self.ep_no += 1
            ...
            obs_opp1 = env_opp1.reset(pose=[8.13253593,0.94485217,0.98489636])
            
            obs_opp = env_opp.reset(pose=[1.26009154,2.40560627,4.17694473])
            env_opp.state.w = (4.17694473-4.15845013)/0.1
            vx_ = (1.26009154-1.52079821)/0.1
            vy_ = (2.40560627-2.83210254)/0.1
            theta_ = 4.17694473
            env_opp.state.vx = np.cos(theta_)*vx_ + np.sin(theta_)*vy_
            env_opp.state.vy = -np.sin(theta_)*vx_ + np.cos(theta_)*vy_
            
            obs = env.reset(pose=[2.57846665,4.07345963,4.12993431])
            env.state.w = (4.12993431-4.1179285)/0.1
            theta_ = 4.12993431
            vx_ = (2.57846665-2.85769844)/0.1
            vy_ = (4.07345963-4.48823595)/0.1
            env.state.vx = np.cos(theta_)*vx_ + np.sin(theta_)*vy_
            env.state.vy = -np.sin(theta_)*vx_ + np.cos(theta_)*vy_
            
            self.curr_sf2 = 0.2
            self.curr_lookahead_factor = 0.3
            self.blocking_factor = 0. #+ len(self.dataset)*0.125*2
            if len(self.dataset) <= 20 :
                self.curr_sf1 = 0.1 + len(self.dataset)*0.01 
                self.curr_speed_factor = 0.85 + len(self.dataset)*0.01
            elif len(self.dataset) <= 40 :
                self.curr_sf1 = 0.3
                self.curr_speed_factor = 0.85 + 0.2
            else :
                t_ = len(self.dataset)-40
                alpha = 0.15
                beta = 0.07
                multiplier = np.cos(t_*alpha)*np.exp(-t_*beta)
                self.curr_sf2 = 0.3 + multiplier*(0.2-0.3)
                self.curr_lookahead_factor = 0.15 + multiplier*(0.3-0.15)
                self.blocking_factor = 0.2 + multiplier*(0.-0.2)
                self.curr_sf1 = 0.25 + multiplier*(0.3-0.25)
                self.curr_speed_factor = 1.0 + multiplier*(1.05-1.0)
                    
            
            self.curr_sf1_opp = 0.3
            self.curr_sf2_opp = 0.1
            self.curr_lookahead_factor_opp = 0.2
            if len(self.dataset) <= 20 :
                self.curr_speed_factor_opp = 0.85
                self.blocking_factor_opp = 0.
            elif len(self.dataset) <= 40 :
                self.curr_speed_factor_opp = 0.85 + (len(self.dataset)-20)*0.01
                self.blocking_factor_opp = 0. + (len(self.dataset)-20)*0.1
            else :
                t_ = len(self.dataset)-40
                alpha = 0.15
                beta = 0.07
                multiplier = np.cos(t_*alpha)*np.exp(-t_*beta)
                self.curr_sf2_opp = 0.15 + multiplier*(0.3-0.15)
                self.curr_lookahead_factor_opp = 0.35 + multiplier*(0.2-0.35)
                self.curr_sf1_opp = 0.1 + multiplier*(0.3-0.1)
                self.curr_speed_factor_opp = 0.97 + multiplier*(1.05-0.97)
                self.blocking_factor_opp = 1. + multiplier*(2.-1.)
        ...
        if self.ep_no > 100 :
            print("Saving dataset")
            pickle.dump(np.array(self.diffs), open(args.exp_name+'_diffs.pkl','wb'))
            pickle.dump(np.array(self.dataset), open(args.exp_name+'.pkl','wb'))
            pickle.dump(np.array(self.ref_trajs), open(args.exp_name+'_plan.pkl','wb'))
            pickle.dump(np.array(self.start_obs), open(args.exp_name+'_start_obs.pkl','wb'))
            exit(0)
        mu_factor = 1.  
        ...
        if SIM == 'numerical':
            ...
            steer, throttle, _, _, self.last_i_opp = self.mpc((px_opp,py_opp,psi_opp),(s_opp,e_opp,vx_opp),(s,e,vx),(s_opp1,e_opp1,vx_opp1),self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp*2,self.curr_speed_factor_opp**2,self.blocking_factor_opp,last_i=self.last_i_opp)
            # steer, throttle, _, _, self.last_i_opp = self.pure_pursuit((px_opp,py_opp,psi_opp),(s_opp,e_opp,vx_opp),(s,e,vx),(s_opp1,e_opp1,vx_opp1),self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp,self.curr_speed_factor_opp,0.,last_i=self.last_i_opp)
            ...
            if self.i == 1 :
                state_obs_start = [s,s_opp,s_opp1, e,e_opp,e_opp1, theta_diff, obs[3], obs[4], obs[5], theta_diff_opp, obs_opp[3], obs_opp[4], obs_opp[5], theta_diff_opp1, obs_opp1[3], obs_opp1[4], obs_opp1[5],curv,curv_opp,curv_opp1,curv_lookahead,curv_opp_lookahead,curv_opp1_lookahead,self.curr_sf1,self.curr_sf2,self.curr_lookahead_factor,self.curr_speed_factor,self.blocking_factor,self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp,self.curr_speed_factor_opp,self.blocking_factor_opp,self.curr_sf1_opp1,self.curr_sf2_opp1,self.curr_lookahead_factor_opp1,self.curr_speed_factor_opp1,0.]
                self.start_obs.append(state_obs_start)
