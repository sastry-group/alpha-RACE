...
parser.add_argument('--exp_name', default='fig3', type=str, help='Name of the experiment')
...
class CarNode(Node):
    ...
    def timer_callback(self):
        ...
        if self.i > EP_LEN :
            ...
            env_opp.state.vx = 3.
            obs = env.reset(pose=self.frenet_to_global([5.1,-0.1,-0.13],waypoint_generator))
            env.state.vx = 4.
            self.curr_sf1 = 0.2 
            self.curr_sf2 = 0.1
            self.curr_lookahead_factor = 0.5 
            self.curr_speed_factor = 0.8 + len(self.dataset)*0.05
