...
# Large label fontsize for better readability
...
data = np.array(np.load('fig1.pkl', allow_pickle=True))[1:]
ref_trajs = np.array(np.load('fig1_plan.pkl', allow_pickle=True))[1:]
theta = 45.*np.pi/180.
...
data_X = np.array(np.load('fig1_start_obs.pkl', allow_pickle=True))[1:]
...
plt.figure()
plt.plot(data_X[1:-1,-15],Y.detach().numpy().flatten()[1:-1],linewidth=3)
plt.xlabel(r'$s_1$')
plt.ylabel('Potential function value')
plt.savefig('fig1_pred.png')
plt.show()