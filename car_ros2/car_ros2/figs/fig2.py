...
parser.add_argument('--exp_name', default='fig2', type=str, help='Name of the experiment')
...
class CarNode(Node):
    ...
    def mpc(self,xyt,pose,pose_opp,pose_opp1,sf1,sf2,lookahead_factor,v_factor,gap=0.06,last_i = -1) :
    ...
    def calc_shift(self,s,s_opp,vs,vs_opp,sf1=0.4,sf2=0.1,t=1.0) :
        ...
        if vs == vs_opp :
            return 0.
        ttc = (s_opp-s)+(vs-vs_opp)*t
        eff_s = ttc 
        factor = sf1*np.exp(-sf2*np.abs(eff_s)**2)
        return factor
    
    def pure_pursuit(self,xyt,pose,pose_opp,pose_opp1,sf1,sf2,lookahead_factor,v_factor,gap=0.06,last_i = -1) :
        s,e,v = pose
        # print(last_i)
        x,y,theta = xyt
        s_opp,e_opp,v_opp = pose_opp
    ...
    def timer_callback(self):
        ...
        if self.i > EP_LEN :
            ...
            obs_opp = env_opp.reset(pose=[3.,5.,-np.pi/2.-0.72])
            # obs_opp = env_opp.reset(pose=self.frenet_to_global([1.3,0.,-0.1],waypoint_generator))
            env_opp.state.vx = 3.
            obs = env.reset(pose=self.frenet_to_global([5.1,0.25,0.],waypoint_generator))
            env.state.vx = 4.
            self.curr_sf1 = 0.2 
            self.curr_sf2 = 0.1
            self.curr_lookahead_factor = 0.1 + len(self.dataset)*0.05
            self.curr_speed_factor = 0.9
            
            self.curr_sf1_opp = 0.
            self.curr_sf2_opp = 0.4
            self.curr_lookahead_factor_opp = 0.25
            self.curr_speed_factor_opp = 0.7
        ...
        if SIM == 'numerical':
            # steer, throttle, ref_traj, _, self.last_i = self.mpc((px,py,psi),(s,e,vx),(s_opp,e_opp,vx_opp),(s_opp1,e_opp1,vx_opp1),self.curr_sf1,self.curr_sf2,self.curr_lookahead_factor*2,self.curr_speed_factor**2,last_i=self.last_i)
            ...
            # steer, throttle, _, _, self.last_i_opp = self.pure_pursuit((px_opp,py_opp,psi_opp),(s_opp,e_opp,vx_opp),(s,e,vx),(s_opp1,e_opp1,vx_opp1),self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp,self.curr_speed_factor_opp,last_i=self.last_i_opp)
            steer=throttle=0.
            if abs(e_opp) > 0.5 :
            ...
            if self.i == 1 :
                state_obs_start = [s,s_opp,s_opp1, e,e_opp,e_opp1, theta_diff, obs[3], obs[4], obs[5], theta_diff_opp, obs_opp[3], obs_opp[4], obs_opp[5], theta_diff_opp1, obs_opp1[3], obs_opp1[4], obs_opp1[5],curv,curv_opp,curv_opp1,curv_lookahead,curv_opp_lookahead,curv_opp1_lookahead,self.curr_sf1,self.curr_sf2,self.curr_lookahead_factor,self.curr_speed_factor,0.,self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp,self.curr_speed_factor_opp,0.,self.curr_sf1_opp1,self.curr_sf2_opp1,self.curr_lookahead_factor_opp1,self.curr_speed_factor_opp1,0.]
