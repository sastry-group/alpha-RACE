...
parser.add_argument('--exp_name', default='fig1', type=str, help='Name of the experiment')
...
class CarNode(Node):
    ...
    def timer_callback(self):
        ...
        if self.i > EP_LEN :
            ...
            obs_opp = env_opp.reset(pose=self.frenet_to_global([3.8,0.,-0.15],waypoint_generator))
            env_opp.state.vx = 0.2
            obs = env.reset(pose=self.frenet_to_global([0.1,0.15,0.],waypoint_generator))
            env.state.vx = 4.
            self.curr_sf1 = 0.2 + len(self.dataset)*0.05
            self.curr_sf2 = 0.1
            ...
            self.curr_lookahead_factor_opp = 0.25
            self.curr_speed_factor_opp = 0.3
