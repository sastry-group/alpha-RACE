...
class CarNode(Node):
    ...
    def timer_callback(self):
        ...
        if self.i > EP_LEN :
            ...
            obs_opp = env_opp.reset(pose=self.frenet_to_global([0.2,0.3,-0.0],waypoint_generator))
            env_opp.state.vx = 2.5
            obs = env.reset(pose=self.frenet_to_global([3.8,0.,0.],waypoint_generator))
            env.state.vx = 1.2
            self.curr_sf1 = 0.1 
            self.curr_sf2 = 0.03
            self.curr_lookahead_factor = 0.3
            self.curr_speed_factor = 0.72
            self.blocking_factor = 0. + len(self.dataset)*0.125*2
            
            self.curr_sf1_opp = 0.3
            self.curr_sf2_opp = 0.1
            self.curr_lookahead_factor_opp = 0.22
        ...
        if SIM == 'numerical':
            ...
            if self.i == 1 :
                state_obs_start = [s,s_opp,s_opp1, e,e_opp,e_opp1, theta_diff, obs[3], obs[4], obs[5], theta_diff_opp, obs_opp[3], obs_opp[4], obs_opp[5], theta_diff_opp1, obs_opp1[3], obs_opp1[4], obs_opp1[5],curv,curv_opp,curv_opp1,curv_lookahead,curv_opp_lookahead,curv_opp1_lookahead,self.curr_sf1,self.curr_sf2,self.curr_lookahead_factor,self.curr_speed_factor,self.blocking_factor,self.curr_sf1_opp,self.curr_sf2_opp,self.curr_lookahead_factor_opp,self.curr_speed_factor_opp,0.,self.curr_sf1_opp1,self.curr_sf2_opp1,self.curr_lookahead_factor_opp1,self.curr_speed_factor_opp1,0.]
