...
V1.load_state_dict(torch.load('model_multi0_4_myopic.pth'))
V2.load_state_dict(torch.load('model_multi1_4_myopic.pth'))
V3.load_state_dict(torch.load('model_multi2_4_myopic.pth'))
...
Y = V1(torch.tensor(data_X).float()) + 35.
# Y[:2,0] -= 6.
print(Y)
...
for i in range(len(data)):
    ...
    color_data = green_cmap(norm(0.+0.125*i))  # Get color for data[i]
    color_data1 = red_cmap(norm(0.+0.125*i))  # Get color for data[i]
    if i == len(data)-1:
...
plt.figure()
plt.plot(data_X[:-1,-11]/2.,Y.detach().numpy().flatten()[1:],linewidth=3)
plt.xlabel(r'$s_3$')
plt.ylabel('Potential function value')
plt.savefig('fig5_pred.png')
plt.show()