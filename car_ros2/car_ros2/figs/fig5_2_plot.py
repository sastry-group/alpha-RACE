...
data = np.array(np.load('fig5_2.pkl', allow_pickle=True))[1:]
ref_trajs = np.array(np.load('fig5_2_plan.pkl', allow_pickle=True))[1:]
theta = 45.*np.pi/180.
...
for i in range(len(ref_trajs)):
    ...
    if i == 0:
        plt.plot(ref_trajs[i, :, 0], ref_trajs[i, :, 1], label='low b, low s2')
    elif i==1 :
        plt.plot(ref_trajs[i, :, 0], ref_trajs[i, :, 1], label='low b, high s2')
    elif i==2 :
        plt.plot(ref_trajs[i, :, 0], ref_trajs[i, :, 1], label='high b, low s2')
    elif i==3 :
        plt.plot(ref_trajs[i, :, 0], ref_trajs[i, :, 1], label='high b, high s2')
