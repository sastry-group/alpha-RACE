...
Y = -np.maximum(X[:,1:,2],X[:,1:,1]) + np.maximum(X[:,:-1,2],X[:,:-1,1]) #- 2.*(np.abs(data[:,:-1,2])>0.5)*(np.abs(data[:,:-1,2])-0.5) #- 0.3*(dist_opp<0.25)
Y_ = -np.maximum(X[:,1:,0],-X[:,1:,1]) + np.maximum(X[:,:-1,0],-X[:,:-1,1]) #- 2.*(np.abs(data[:,:-1,3])>0.5)*(np.abs(data[:,:-1,3])-0.5) #- 0.3*(dist_opp<0.25)
Y__ = -np.maximum(-X[:,1:,2],-X[:,1:,0]) + np.maximum(-X[:,:-1,2],-X[:,:-1,0]) #- 2.*(np.abs(data[:,:-1,3])>0.5)*(np.abs(data[:,:-1,3])-0.5) #- 0.3*(dist_opp<0.25)
...
for i in range(n_iters) :
    ...
    if i%1000 == 0 :
        torch.save(model.state_dict(), 'q_models_rel/model_multi0'+str(suffix)+'.pth')
        torch.save(model_.state_dict(), 'q_models_rel/model_multi1'+str(suffix)+'.pth')
        torch.save(model__.state_dict(), 'q_models_rel/model_multi2'+str(suffix)+'.pth')
