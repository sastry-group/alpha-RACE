...
data_X = np.array(np.load('fig5_start_obs.pkl', allow_pickle=True))[1:]
a = 1700
b = 2200
plt.plot(waypoint_generator.right_boundary[a:b,0], waypoint_generator.right_boundary[a:b,1], 'k',label='track boundary')
plt.plot(waypoint_generator.left_boundary[a:b,0], waypoint_generator.left_boundary[a:b,1], 'k')
plt.plot(waypoint_generator.raceline[a:b,0], waypoint_generator.raceline[a:b,1], '--',label='raceline')
...
for i in range(2,N):
    ...
    data[i, :, 3:5] = np.matmul(data[i, :, 3:5], R)
    # if i==len(data)-1 :
    #     print(data[-1,:,:2])

    color_data = green_cmap(norm(0.+i/N))  # Get color for data[i]
    color_data1 = red_cmap(norm(0.+i/N))  # Get color for data[i]
    if i == len(data)-1:
        plt.plot(data[i, :, 0], data[i, :, 1], color=color_data,label='ego trajectory')
        plt.plot(data[i, :, 3], data[i, :, 4], color=color_data1,label='opp trajectory')
    else :
        plt.plot(data[i, :, 0], data[i, :, 1], color=color_data)
        plt.plot(data[i, :, 3], data[i, :, 4], color=color_data1)
...
cbar_red = plt.colorbar(sm_red, ax=plt.gca(), orientation='vertical', fraction=0.025, pad=0.04)
