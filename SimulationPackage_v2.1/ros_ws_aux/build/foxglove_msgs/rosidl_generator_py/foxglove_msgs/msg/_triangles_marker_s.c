...
bool foxglove_msgs__msg__triangles_marker__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("foxglove_msgs.msg._triangles_marker.TrianglesMarker", full_classname_dest, 51) == 0);
  }
  foxglove_msgs__msg__TrianglesMarker * ros_message = _ros_message;
  {  // timestamp
  ...
  {  // colors
    ...
    foxglove_msgs__msg__Color * dest = ros_message->colors.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
  ...
  {  // indices
    PyObject * field = PyObject_GetAttrString(_pymsg, "indices");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      uint32_t * dest = ros_message->indices.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'indices'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of TrianglesMarker */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("foxglove_msgs.msg._triangles_marker");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "TrianglesMarker");
  ...
  {  // indices
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "indices");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->indices.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint32_t * src = &(ros_message->indices.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->indices.size * sizeof(uint32_t));
      assert(data != NULL);
