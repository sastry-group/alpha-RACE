...
bool foxglove_msgs__msg__laser_scan__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("foxglove_msgs.msg._laser_scan.LaserScan", full_classname_dest, 39) == 0);
  }
  foxglove_msgs__msg__LaserScan * ros_message = _ros_message;
  {  // timestamp
  ...
  {  // ranges
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->ranges.data;
  ...
  {  // intensities
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->intensities.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * foxglove_msgs__msg__laser_scan__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("foxglove_msgs.msg._laser_scan");
  ...
  {  // ranges
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "ranges");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->ranges.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->ranges.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->ranges.size * sizeof(double));
      assert(data != NULL);
  ...
  {  // intensities
    ...
    if (ros_message->intensities.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->intensities.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->intensities.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
